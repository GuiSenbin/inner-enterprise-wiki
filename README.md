...
8. API 返回 `intent_type`、`retrieval_strategy`、`result_mode`、`candidate_top_k`、`evidence_limit`、`total_related_projects`、`rerank_explanation`、`evidence_status`、`confidence_score` 和 `confidence_label`；证据数量按问题意图动态决定。
9. 页面展示索引状态、切片数、图谱节点、双链关系、编译时间、问题意图、检索策略、结果范围、证据置信度、最终回答和证据轨道。
...
这一步让系统先判断“该怎么找资料”，而不是所有问题都走同一个固定 Top K 向量召回。实体画像和跨项目归纳会进入全量关联模式，明确文档定位会进入精准模式，普通问答才使用 Top5。

### 2.1 动态结果范围

系统将“候选召回数量”和“最终证据数量”分开：

| 结果模式 | 典型问题 | 候选召回 | 最终证据 | 处理规则 |
| --- | --- | ---: | ---: | --- |
| `entity_full` | “刘洋都负责哪些项目？” | 50 | 50 | 图谱全量关联，按项目去重 |
| `project_full` | “哪些项目提到了主要风险？” | 50 | 50 | 跨项目召回，按项目去重 |
| `precise` | “投研数据中台的验收指标有哪些？” | 10 | 2 | 优先目标实体和目标文档类型 |
| `top_k` | 普通 Wiki 问题 | 15 | 5 | 混合召回后取 Top5 |

全量模式设置 50 条安全上限，避免无界查询拖垮上下文；同一项目的技术方案、项目管理计划等多份文档只保留综合分最高的一条证据。
...
每条最终证据都会返回 `rerank_reasons`，前端能展示“为什么这条证据排在前面”。

### 5. 有证据回答与拒答

`AnswerService` 负责把动态结果模式选出的证据组装为“已编译 Wiki 证据”，再调用通义千问 OpenAI 兼容接口生成中文回答。Prompt 明确要求：
...
目标是把原型收敛为可演示、可核对的 RAG 主链路。该阶段缩减并规范文档集为 12 个项目、5 类文档、60 份 Raw 资料，补充 Raw/Wiki 双链、FAISS 向量索引、图谱增强、Prompt 展示和原生 Web 页面。
...
7. 检索评估集：建立固定问题、期望来源、不同结果模式下的召回覆盖率、引用准确率和人工评分。
