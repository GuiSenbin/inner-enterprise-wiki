...
{query}
        ...
        请严格按下面格式输出，结论必须放在回答第一部分，不要先写过程或泛泛说明：

## 结论
用 1-3 句话直接回答用户的问题；如果问题要求列举对象，请先列出对象名称。

## 证据依据
只引用已编译 Wiki 证据中明确出现的事实，并注明对应文档。

## 补充说明
只有在确实有必要时补充范围、时间或不确定性；没有补充内容时省略本节。

不要输出“答案依据：”“答案：”等旧式标签，不要重复用户问题，不要把检索过程当成结论。"""
